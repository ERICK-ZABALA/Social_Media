#!/usr/bin/env python3
"""Generador diario de Insight Star (liderazgo / desarrollo personal) para Rock Factory.

Igual que generar_retro_cartoon.py pero lee INSIGHTS (titulo + lineas, una por
toma) de playlist/insights.json. El video queda MUDO (audio se elige en TikTok).

Uso:
  python3 generar_insight_star.py [--dia N] [--out salida.mp4] [--segments 3] \
      [--insight "linea1|linea2|linea3"]
  Si no pasas --insight, lo lee de playlist/insights.json segun el dia.
...
CH = BASE / "channels" / "insight-star"
ESTADO = CH / "estado.json"
INSIGHTS = CH / "playlist" / "insights.json"
BG = CH / "assets" / "bg"
MEDIA = CH / "media"
...
def pick_insight(dia: int, explicit=None) -> tuple[list[str], str]:
    if explicit:
        return [l.strip() for l in explicit.split("|") if l.strip()], "custom"
    data = json.loads(INSIGHTS.read_text(encoding="utf-8"))
    items = data["insights"]
    if not items:
        raise GenError("no hay insights en insights.json")
    it = items[dia % len(items)]
    return it["lineas"], it.get("titulo", "Insight")
...
def render(lineas, titulo, out: Path, segs: int) -> Path:
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="insight_"))
    clips = []
    try:
        for i, fr in enumerate(per):
            ...
            chain = [
                ...
                "eq=contrast=1.06:saturation=1.02:brightness=0.04",
                "vignette=PI/4.4",
            ]
            if linea:
                words = linea.split()
...
def main():
    ...
    ap.add_argument("--insight", type=str, default=None,
                    help="lineas separadas por |")
    a = ap.parse_args()

    estado = load_estado()
    dia = a.dia if a.dia is not None else estado.get("dia", 0) + 1
    lineas, titulo = pick_insight(dia, a.insight)
