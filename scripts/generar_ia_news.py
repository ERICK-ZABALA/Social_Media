#!/usr/bin/env python3
"""Generador diario de IA Generativa News (avances/novedades de IA) para Rock Factory.

Igual que generar_retro_cartoon.py pero lee NOTICIAS (titulo + lineas, una por
toma) de playlist/noticias.json. El video queda MUDO (audio se elige en TikTok).

Uso:
  python3 generar_ia_news.py [--dia N] [--out salida.mp4] [--segments 3] \
      [--noticia "linea1|linea2|linea3"]
  Si no pasas --noticia, la lee de playlist/noticias.json segun el dia.

NOTA: las imagenes de assets/bg/toma_*.png son placeholders de gradiente;
reemplazalas por fondos reales del canal cuando los tengas.
"""
...
CH = BASE / "channels" / "ia-generativa-news"
ESTADO = CH / "estado.json"
NOTICIAS = CH / "playlist" / "noticias.json"
BG = CH / "assets" / "bg"
MEDIA = CH / "media"
...
def pick_noticia(dia: int, explicit=None) -> tuple[list[str], str]:
    if explicit:
        return [l.strip() for l in explicit.split("|") if l.strip()], "custom"
    data = json.loads(NOTICIAS.read_text(encoding="utf-8"))
    items = data["noticias"]
    if not items:
        raise GenError("no hay noticias en noticias.json")
    it = items[dia % len(items)]
    return it["lineas"], it.get("titulo", "IA News")
...
def render(lineas, titulo, out: Path, segs: int) -> Path:
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="ianews_"))
    clips = []
    try:
        for i, fr in enumerate(per):
            ...
            chain = [
                ...
                "eq=contrast=1.10:saturation=1.15:brightness=0.03",
                "vignette=PI/4.4",
            ]
            if linea:
                words = linea.split()
                ...
                chain.append(
                    ...
                    f"fontsize=42:line_spacing=8:x=(w-text_w)/2:y={y_top}:"
            ...
            chain += [
                ...
                f"drawtext=fontfile={fon}:text='{esc(BRAND)}':fontcolor=0x00E5FF:"
                f"fontsize=24:x=60:y={y_brand}:shadowcolor=black@0.8:shadowx=2:shadowy=2:alpha='{fade_e}'",
            ]
            fc = ",".join(chain)
...
def main():
    ...
    ap.add_argument("--noticia", type=str, default=None,
                    help="lineas separadas por |")
    a = ap.parse_args()

    estado = load_estado()
    dia = a.dia if a.dia is not None else estado.get("dia", 0) + 1
    lineas, titulo = pick_noticia(dia, a.noticia)
